...
The **Production-Grade LoRA Training Pipeline** implements end-to-end autonomous model fine-tuning with GPU quota management:
- **Auto-Training Trigger**: Monitors user interactions and semantic query patterns to automatically create training jobs when sufficient data accumulates in namespaces.
- **Dataset Preprocessing**: Converts curated knowledge base entries into HuggingFace Dataset format with proper tokenization and formatting for instruction-tuning.
- **Kaggle Worker**: Automated Jupyter notebook creation and execution via `KaggleAutomationService` with Puppeteer-based browser automation, stealth anti-detection, session persistence, and code injection.
- **LoRA Training**: Fine-tunes TinyLlama-1.1B-Chat (600MB) using 4-bit quantization (BitsAndBytes), LoRA adapters (PEFT), fp16 mixed precision, gradient accumulation, and real HuggingFace Trainer with DataCollator.
- **Model Persistence**: Saves LoRA adapter weights to `/kaggle/working/lora_adapters` with `model.save_pretrained()` and `tokenizer.save_pretrained()`.
- **Metrics Tracking**: Reports training loss, validation metrics, and completion status to backend via `/api/training/jobs/:id/complete`.
- **Upgrade Path**: Phase 1 (current): TinyLlama on Kaggle validates pipeline. Phase 2: Modal/RunPod with Llama-3.2-8B. Phase 3: Llama-3.3-70B production.
- **Schema**: `training_jobs` table includes `dataset_id` (FK to datasets), `config` (JSONB), `model` (TEXT), `metrics` (JSONB), `error_message` (TEXT), with status defaulting to "queued".
