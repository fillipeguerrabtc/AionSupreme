...
export function registerCurationRoutes(app: Router) {
  ...
  app.post("/curation/add", async (req, res) => {
    try {
      ...
      // NOTE: DuplicateContentError is already caught in outer catch block (line 114)
      ...
      // 🔥 P1.1.2g: Handle duplicate errors + persist rejection metadata via centralized helper
      if (error.name === 'DuplicateContentError') {
        try {
          // Use centralized helper with original payload data
          await curationStore.persistRejection({
            title,
            content,
            suggestedNamespaces,
            tags,
            submittedBy,
            // contentHash/normalizedContent auto-generated by helper
          }, error);
        } catch (persistError: any) {
          console.error(`[Curation API] ⚠️ Failed to persist rejection metadata:`, persistError.message);
          // Continue - still return 409 to client
        }
