...
export class GpuLoadBalancer {
  ...
  /**
   * Execute DeepWeb search on GPU worker (via Tor)
   */
  async executeDeepWebSearch(
    query: string,
    ...
    // Verify Tor is enabled
    if (!worker.capabilities.tor_enabled) {
      console.warn(`[Load Balancer] Worker ${worker.provider} does not have Tor enabled`);
      return {
        success: false,
        error: "Selected worker does not support Tor",
      };
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${worker.ngrokUrl}/deepweb/search`,
        {
          query,
          max_results: maxResults,
        },
        {
          timeout: 60000, // 60s for Tor searches (slower)
          headers: {
            "Content-Type": "application/json",
          },
        }
      ...
      console.log(`[Load Balancer] DeepWeb search completed in ${latencyMs}ms via ${worker.provider}`);
      ...
      console.error(`[Load Balancer] Error executing DeepWeb search on ${worker.provider}:`, error.message);
