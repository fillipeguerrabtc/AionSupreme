 ...
 */
onEvent("AGENT_NAMESPACES_CHANGED", async ({ tenantId, agentId, namespaces }) => {
  ...
  for (const namespace of namespaces) {
    try {
      ...
      console.log(`[AgentsIndexer] Rebuilding namespace: ${namespace} for tenant ${tenantId}`);
 ...
 */
onEvent("DOC_INGESTED", async ({ tenantId, documentId, namespace }) => {
