...
export default function AgentsPage() {
  ...
  const orphanScanMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast({ 
        title: t.admin.agents.orphanScan.scanComplete, 
        description: t.admin.agents.orphanScan.orphansDetectedTemplate.replace('{{count}}', data.report.totalOrphans.toString())
      });
    },
    onError: (error: Error) => {
      toast({ 
        title: t.admin.agents.orphanScan.scanError, 
  ...
  return (
    <div className="space-y-6 max-w-full overflow-x-hidden">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="list" className="space-y-4 mt-4">
          <div className="flex items-center justify-end gap-2 flex-wrap">
            <Button 
              ...
              title={t.admin.agents.orphanScan.buttonTooltip}
            >
              <Search className="w-4 h-4 mr-2" />
              {orphanScanMutation.isPending ? t.admin.agents.orphanScan.scanning : t.admin.agents.orphanScan.diagnoseIntegrity}
            </Button>
          </div>
        ...
        <CardContent>
          {isLoading ? (
            <div className="text-center py-8 text-muted-foreground">{t.admin.agents.loading}</div>
          ) : agents.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              {t.admin.agents.noAgentsDesc}
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t.admin.agents.table.icons}</TableHead>
                  <TableHead>{t.admin.agents.table.name}</TableHead>
                  <TableHead>{t.admin.agents.table.slug}</TableHead>
                  <TableHead>{t.admin.agents.table.type}</TableHead>
                  <TableHead>{t.admin.agents.table.namespaces}</TableHead>
                  <TableHead className="text-right">{t.admin.agents.table.actions}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {agents.map((agent) => (
      ...
      <Dialog open={selectedAgent !== null} onOpenChange={(open) => !open && setSelectedAgent(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t.admin.agents.editDialog.title}</DialogTitle>
            <DialogDescription>
              {t.admin.agents.editDialog.subtitleTemplate.replace('{{name}}', selectedAgent?.name || '')}
            </DialogDescription>
          </DialogHeader>
          {selectedAgent && (
            <div className="space-y-4">
              <div>
                <Label htmlFor="edit-name">{t.admin.agents.editDialog.agentName}</Label>
                <Input 
                  id="edit-name" 
                  value={editName}
              ...
              <div>
                <Label>{t.admin.agents.editDialog.slug}</Label>
                <div className="px-3 py-2 bg-muted rounded-md">
                  <code className="text-sm font-mono">{editSlug}</code>
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {t.admin.agents.editDialog.slugAutoGenerated}
                </p>
              </div>
              <div>
                <Label htmlFor="edit-description">{t.admin.agents.editDialog.description}</Label>
                <Input 
                  id="edit-description" 
              ...
              <div>
                <Label htmlFor="edit-systemPrompt">{t.admin.agents.editDialog.systemPrompt}</Label>
                <Textarea
              ...
              {selectedAgent.agentTier === "agent" ? (
                <div>
                  <Label htmlFor="edit-namespace">{t.admin.agents.editDialog.rootNamespace}</Label>
                  <p className="text-sm text-muted-foreground mb-2">
                    {t.admin.agents.editDialog.rootNamespaceHelp}
                  </p>
                  <NamespaceSelector 
                    value={editNamespaces} 
                    onChange={(namespaces) => setEditNamespaces(namespaces.slice(0, 1))}
                    placeholder={t.admin.agents.editDialog.rootNamespacePlaceholder}
                    allowCustom={false}
                    allowWildcard={false}
                  />
                </div>
                ...
                <div>
                  <Label htmlFor="edit-namespaces">{t.admin.agents.editDialog.subnamespaces}</Label>
                  <p className="text-sm text-muted-foreground mb-2">
                    {t.admin.agents.editDialog.subnamespacesHelp}
                  </p>
                  <NamespaceSelector 
                    value={editNamespaces} 
                    onChange={setEditNamespaces}
                    placeholder={t.admin.agents.editDialog.subnamespacesPlaceholder}
                    allowCustom={false}
                    allowWildcard={false}
                  />
                </div>
              ...
              <div className="flex justify-end gap-2">
                <Button type="button" variant="outline" onClick={() => setSelectedAgent(null)}>
                  {t.admin.agents.editDialog.cancel}
                </Button>
                <Button onClick={handleSaveEdit} disabled={updateMutation.isPending} data-testid="button-save-agent">
                  {updateMutation.isPending ? t.admin.agents.editDialog.updating : t.admin.agents.editDialog.saveChanges}
                </Button>
              </div>
            </div>
          )}
      ...
      <AlertDialog open={!!deleteAgentId} onOpenChange={(open) => !open && setDeleteAgentId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t.admin.agents.deleteDialog.title}</AlertDialogTitle>
            <AlertDialogDescription>
              {t.admin.agents.deleteDialog.description}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel data-testid="button-cancel-delete">{t.admin.agents.deleteDialog.cancel}</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
                if (deleteAgentId) {
            ...
            >
              {t.admin.agents.deleteDialog.delete}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <Dialog open={showOrphanScanDialog} onOpenChange={setShowOrphanScanDialog}>
        <DialogContent className="max-w-4xl max-h-[80vh]">
          <DialogHeader>
            <DialogTitle>{t.admin.agents.orphanDialog.title}</DialogTitle>
            <DialogDescription>
              {t.admin.agents.orphanDialog.description}
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <Card>
                    <CardContent className="pt-6">
                      ...
                      <p className="text-xs text-muted-foreground">{t.admin.agents.orphanDialog.severityHigh}</p>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="pt-6">
                      <div className="text-2xl font-bold text-yellow-600">{orphanScanResult.report.summary.medium}</div>
                      <p className="text-xs text-muted-foreground">{t.admin.agents.orphanDialog.severityMedium}</p>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="pt-6">
                      <div className="text-2xl font-bold text-green-600">{orphanScanResult.report.summary.low}</div>
                      <p className="text-xs text-muted-foreground">{t.admin.agents.orphanDialog.severityLow}</p>
                    </CardContent>
                  </Card>
                </div>
                    ...
                    <Card key={module.module}>
                      <CardHeader>
                        <CardTitle className="text-lg flex items-center justify-between">
                          <span>{t.admin.agents.orphanDialog.moduleLabel}: {module.module}</span>
                          <Badge variant="secondary">{t.admin.agents.orphanDialog.orphansTemplate.replace('{{count}}', module.totalOrphans.toString())}</Badge>
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                        <div className="space-y-3">
                          {module.orphans.map((orphan: any, idx: number) => (
                            <div key={idx} className="border-l-4 pl-3 py-2" style={{
                              ...
                              <p className="text-xs text-muted-foreground mt-1">
                                <strong>{t.admin.agents.orphanDialog.suggestedAction}</strong> {orphan.suggestedAction}
                              </p>
                            </div>
                          ))}
                        </div>
                ...
                {orphanScanResult.report.totalOrphans === 0 && (
                  <Card>
                    <CardContent className="pt-6 text-center">
                      <p className="text-lg text-green-600 font-semibold">{t.admin.agents.orphanDialog.noOrphans}</p>
                      <p className="text-sm text-muted-foreground mt-1">
                        {t.admin.agents.orphanDialog.allHealthy}
                      </p>
                    </CardContent>
                  </Card>
                )}
